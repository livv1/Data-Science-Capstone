...
def get_pie_chart(site_dropdown):
    if site_dropdown == 'ALL':
        ...
        fig = px.pie(data_frame = filter_df, names='class',title='Total Launch for a Specific Site')
        return fig 
...
def update_graph(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        ...
        fig1 = px.scatter(data_frame=filtered_data, x="Payload Mass (kg)", y="class", 
        color="Booster Version Category")
        return fig1
    else:
        filter_df=spacex_df.loc[spacex_df['Launch Site'] == site_dropdown]
        filtered_data = filter_df[(filter_df['Payload Mass (kg)']>=payload_slider[0])
        &(spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        fig1 = px.scatter(data_frame=filtered_data, x="Payload Mass (kg)", y="class", 
        color="Booster Version Category")
        return fig1
